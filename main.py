...
vn30 = ['ACB', 'BID', 'BVH', 'CTG', 'FPT', 'GAS', 'GVR', 'HDB', 'HPG', 'KDH', 'MBB', 'MSN', 'MWG', 'NVL', 'PDR', 'PLX',
        'PNJ', 'POW', 'SAB', 'SSI', 'STB', 'TCB', 'TPB', 'VCB', 'VHM', 'VIC', 'VJC', 'VNM', 'VPB', 'VRE']
country = 'VietNam'

for symbol in vn30:
    print(f'Load data: {symbol}')
    df = investpy.get_stock_historical_data(
        stock=symbol, country=country, from_date=start, to_date=end)
    df = pd.DataFrame(df)
    ...
    si = investpy.get_stock_information(stock=symbol, country=country)
    si = pd.DataFrame(si)
    si.to_csv(f'{path}{symbol}i.csv', index=False)
