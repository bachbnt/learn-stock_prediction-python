...
class StockHistoricalData:
    ...
    def get_data(self):
        ...
        df[f'{const.sma}{const.ma1}'] = df['Close'].rolling(window=const.ma1).mean()
        df[f'{const.sma}{const.ma2}'] = df['Close'].rolling(window=const.ma2).mean()
        df[f'{const.sma}{const.ma3}'] = df['Close'].rolling(window=const.ma3).mean()
